...
func getCase(c rune) {
	...
	switch c {
	case '1':
		changeValue("-{088e3905-0323-4b02-9826-5d99428e115f}")
		fmt.Println("Download is disabled")
		break
	case '2':
		changeValue("-{0DB7E03F-FC29-4DC6-9020-FF41B59E513A}") //or {31C0DD25-9439-4F12-BF41-7FF4EDA38722}
		fmt.Println("3D Objects Disabled")
		break
	case '3':
		changeValue("-{24ad3ad4-a569-4530-98e1-ab02f9417aa8}")
		fmt.Println("Pictures is disabled")
		break
	case '4':
		changeValue("-{B4BFCC3A-DB2C-424C-B029-7FE99A87C641}")
		fmt.Println("Desktop is disabled")
		break
	case '5':
		changeValue("-{d3162b92-9365-467a-956b-92703aca08af}")
		fmt.Println("Documents is disabled")
		break
	case '6':
		changeValue("-{3dfdf296-dbec-4fb4-81d1-6a3438bcf4de}")
		fmt.Println("Music is disabled")
		break
	case '7':
		changeValue("-{f86fa3ab-70d2-4fc7-9c99-fcbf05467f3a}")
...
func changeValue(value string) {
	...
	k, _, err := registry.CreateKey(registry.LOCAL_MACHINE, `SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\MyComputer\NameSpace`+value, registry.ALL_ACCESS)
	if err != nil {
		log.Fatal(err)
	}
	v, _, err := registry.CreateKey(registry.LOCAL_MACHINE, `SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Explorer\MyComputer\NameSpace`+value, registry.ALL_ACCESS)
	if err != nil {
		log.Fatal(err)
	}
	k.Close()
